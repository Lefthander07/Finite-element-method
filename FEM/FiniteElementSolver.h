...
public:
	...
	Mesh<Degree>* mesh;
	GlobalSystem<Degree>* globalSystem;

public:
	FiniteElementSolver(SecondOrderDifferentialEq& eq) : equation(eq) {}
	void GenerateMesh(int numberOfElements, double x_start, double x_end);
	void AssembleGlobalMatrix();
	void ApplyBoundaryConditions(std::pair<BoundaryConditions, double>&, std::pair<BoundaryConditions, double>&);
	void SolveSystem();
...
{
    globalSystem = new GlobalSystem<Degree>(mesh->Nodes.size());
